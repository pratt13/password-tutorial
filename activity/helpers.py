...
def bad_password_generator():
    ...
    and endings being combination of numbers and special characters as the last 2
...
def is_valid_password(
    ...
    """
    Given a password check the following conditions if the flag is set
    * Min length
    * Maximum length
    * Check lowercase letter is present
    * Check uppercase letter is present
    * Check number is present
    * Check special character is present
    * Check no common pattern is present
    """
...
def level_one(
    ...
    """
    Assert the password meets some character requirements
    """
    is_valid_password(
        password,
...
def level_two(password):
    """
    This is actually equivalent to checking that the password has all the characters
    So this test is automatically passed, if the previous ones are.
    """
...
def level_three(password):
    """
    The entropy test is essentially, level one, ensure no character is used more than
    three times.
    """
...
def level_four(passwords):
    """
    Test every batch of 100 passwords each is unique.
    """
